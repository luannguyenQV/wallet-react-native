...
class AccountB extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillMount() {
    let i = 0,
      j = 0;
    let zarAccount = this.state.currencies.filter(
      account => account.currency.code === this.state.code,
    );
    this.setState({
      balance: this.setBalance(
        zarAccount[0].available_balance,
        zarAccount[0].currency.divisibility,
      ),
