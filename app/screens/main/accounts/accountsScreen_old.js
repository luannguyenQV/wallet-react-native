  ...
  getSelectedCurrencyBalance = async code => {
    ...
    if (companyCurrencyResponse.status === 'success') {
      ...
      for (let i = 0; i < length; i++) {
        let responseJson = await AccountService.getSelectedCurrency(
          uniqueCompanyCurrencyResponse[i].code,
        );
  ...
  getAccountList = async code => {
    ...
    if (responseJson.status === 'success') {
      ...
      for (i = 0; i < data.length; i++) {
        account = data[i].currencies.filter(
          account => account.currency.code === code,
        );
        if (account.length > 0) {
      ...
      if (account.length > 0) {
        ...
        Alert.alert('Error', 'No account available for this currency.', [
          {
            text: 'OK',
            onPress: () => {
              this.setState({
  ...
  render() {
    return (
      <View style={styles.container}>
        <Header navigation={this.props.navigation} drawer title="Currencies" />
        <ScrollView
