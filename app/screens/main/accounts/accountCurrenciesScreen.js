...
class AccountCurrenciesScreen extends Component {
  ...
  setDataInListView = responseJson => {
    if (responseJson.status === 'success') {
      ...
      Alert.alert('Error', responseJson.message, 'Ok');
    }
  };
  ...
  loadMoreData = async () => {
    if (this.state.refreshing !== true) {
      ...
      let responseJson = await AccountService.getMoreAccountCurrencies(
        this.state.nextUrl,
      );
      this.setDataInListView(responseJson);
    }
  ...
  reload = () => {
    ResetNavigation.dispatchUnderDrawerWithParams(
  ...
  render() {
    return (
      <View style={styles.container}>
        <Header navigation={this.props.navigation} back title="Currencies" />
        <ListView
          ...
          renderRow={rowData => (
            <Currency setActive={this.setActive} data={rowData} />
          )}
...
export default AccountCurrenciesScreen;
